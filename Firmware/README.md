...
`IDENTIFY` lasts ten seconds and temporarily overrides every other indication.
...
cause is available through the controller and web UI. In `GAME`, every node LED
is forced off, including Identify, error, and broken-beam indications, so LEDs
cannot affect an LDR or distract the player. Identify is therefore available
only in setup mode.
...
- an Identify action that flashes the selected physical node for ten seconds;
