...
async function pollGame(){if(busy)return;try{const g=await get('/api/game'),info=stateText(g);$('#state').textContent=info[0];$('#state').className='state '+info[1];const accepts=g.state==='WAIT_PLAYER';$('#player').disabled=!accepts;$('#submitPlayer').disabled=!accepts||!$('#player').value.trim();if(previousState!==null&&previousState!==g.state&&g.state==='WAIT_PLAYER')loadResults();previousState=g.state}catch(e){$('#state').textContent='Controller unavailable: '+e.message;$('#state').className='state bad'}}
$('#player').oninput=()=>$('#submitPlayer').disabled=previousState!=='WAIT_PLAYER'||!$('#player').value.trim();$('#playerForm').onsubmit=async e=>{e.preventDefault();if(busy)return;busy=true;$('#submitPlayer').disabled=true;try{const body=new URLSearchParams(new FormData(e.target));await get('/api/game/player',{method:'POST',body});$('#message').textContent='';$('#player').value=''}catch(e){$('#message').textContent=e.message}finally{busy=false;pollGame()}};
