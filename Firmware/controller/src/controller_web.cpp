...
<section class="card"><h2>Configure all lasers</h2><div id="sensorConsistency" class="status warn">Checking laser configurations…</div><form id="allSensors"><div class="grid"><label>Threshold<input name="threshold" type="number" min="0" max="1023" required value="240"></label><label>Hysteresis<input name="hysteresis" type="number" min="0" max="1023" required value="16"></label><label>Stable time (ms)<input name="stable_time_ms" type="number" min="0" max="1000" required value="30"></label><label>Cooldown (ms)<input name="cooldown_ms" type="number" min="0" max="5000" required value="500"></label></div><button type="submit">Apply to all lasers</button></form></section>
<section class="card"><h2>Node inventory</h2><div class="nodes"><div class="node-head"><span>Address</span><span>Type</span><span>State</span><span>Availability</span><span>Actions</span></div><div id="nodes"><p>Loading…</p></div></div></section></main><dialog id="nodeDialog"><h2 id="dialogTitle"></h2><div id="dialogContent"></div><div class="dialog-actions"><button id="closeDialog" class="secondary">Close</button></div></dialog>
<script>
const $=s=>document.querySelector(s), msg=$('#message');let setup=true,currentSettings=null,currentNodes=[],busy=false;
...
function settingsBody(){const p=new URLSearchParams(new FormData($('#gameConfig')));for(const [k,v] of new FormData($('#wifiConfig')))p.set(k,v);return p}$('#gameConfig').onsubmit=e=>{e.preventDefault();act('/api/settings',{method:'POST',body:settingsBody()},'Game configuration saved')};$('#wifiConfig').onsubmit=e=>{e.preventDefault();act('/api/settings',{method:'POST',body:settingsBody()},'Wi-Fi configuration saved')};$('#allSensors').onsubmit=e=>{e.preventDefault();if(confirm('Apply these settings to every discovered laser node?'))configureAll(e.target)};$('#closeDialog').onclick=()=>$('#nodeDialog').close();refresh();setInterval(refreshNodesLive,2000);setInterval(refreshSystemLive,10000);
</script></body></html>)HTML";

const char GAME_PAGE[] PROGMEM = R"HTML(<!doctype html><html lang="en"><head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Laser Parkour Game</title><style>body{font-family:system-ui,sans-serif;max-width:48rem;margin:4rem auto;padding:0 1.25rem;background:#111827;color:#f9fafb}main{background:#1f2937;border-radius:1rem;padding:2rem}button{padding:.65rem .9rem;border:0;border-radius:.4rem;background:#b91c1c;color:white;cursor:pointer}.status{color:#fbbf24}</style></head>
<body><main><h1>Game</h1><p>The player and scoreboard interface will be implemented in Phase 4.5.</p><button id="returnSetup">Return system to Setup</button><p id="status" class="status"></p></main><script>
document.getElementById('returnSetup').onclick=async()=>{if(!confirm('Return to Setup? An active attempt will be aborted.'))return;const status=document.getElementById('status');status.textContent='Returning to Setup…';try{const r=await fetch('/api/system/setup',{method:'POST',cache:'no-store'});if(!r.ok){const d=await r.json();throw new Error(d.message||d.error||('HTTP '+r.status))}location.replace('/setup')}catch(e){status.textContent=e.message}};
</script></body></html>)HTML";
