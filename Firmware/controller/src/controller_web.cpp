...
<section class="card"><h2>Configure all lasers</h2><div id="sensorConsistency" class="status warn">Checking laser configurations…</div><form id="allSensors"><div class="grid"><label>Threshold<input name="threshold" type="number" min="0" max="1023" required value="240"></label><label>Hysteresis<input name="hysteresis" type="number" min="0" max="1023" required value="16"></label><label>Stable time (ms)<input name="stable_time_ms" type="number" min="0" max="1000" required value="30"></label><label>Cooldown (ms)<input name="cooldown_ms" type="number" min="0" max="5000" required value="500"></label></div><button type="submit">Apply to all lasers</button><button id="resetLaserCounters" type="button" class="secondary">Reset all laser event counters</button><button id="rescan" type="button" class="secondary">Rebuild inventory</button></form></section>
<section class="card"><h2>Node inventory</h2><div id="inventorySummary" class="status">Loading inventory summary…</div><div class="nodes"><div class="node-head"><span>Address</span><span>Type</span><span>State</span><span>Events</span><span>Availability</span><span>Actions</span></div><div id="nodes"><p>Loading…</p></div></div></section></main><dialog id="nodeDialog"><h2 id="dialogTitle"></h2><div id="dialogContent"></div><div class="dialog-actions"><button id="closeDialog" class="secondary">Close</button></div></dialog>
<script>
...
function timestamp(r){if(!r||!r.timestamp_valid)return 'Time unknown';const localMs=(Number(r.completion_unix_s)+Number(r.utc_offset_minutes)*60)*1000,nowLocal=Date.now()-new Date().getTimezoneOffset()*60000,d=new Date(localMs),day=Math.floor(localMs/86400000),today=Math.floor(nowLocal/86400000),clock=String(d.getUTCHours()).padStart(2,'0')+':'+String(d.getUTCMinutes()).padStart(2,'0');if(day===today)return clock;if(day===today-1)return 'Yesterday '+clock;return String(d.getUTCDate()).padStart(2,'0')+'.'+String(d.getUTCMonth()+1).padStart(2,'0')+'. '+clock}
...
function renderTable(id,results,top){const body=$(id);body.replaceChildren();for(let i=0;i<10;i++){const tr=document.createElement('tr'),r=results[i];for(const value of r?[i+1,r.player,timestamp(r),top?time(r.score_time_us):statusName(r.status),top?time(r.raw_time_us):time(r.score_time_us),r.interruptions]:[i+1,'—','—','—','—','—']){const td=document.createElement('td');td.textContent=value;tr.append(td)}body.append(tr)}}
...
async function syncTime(){try{await get('/api/time',{method:'POST',body:new URLSearchParams({unix_ms:Date.now(),utc_offset_minutes:-new Date().getTimezoneOffset()})})}catch(e){console.warn('Time synchronization failed',e)}}syncTime();loadResults();pollGame();setInterval(pollGame,750);setInterval(syncTime,300000);
</script></body></html>)HTML";
...
void handleTimeSync() {
  String response;
  int status = 400;
  if (!server.hasArg("unix_ms") || !server.hasArg("utc_offset_minutes")) {
    response = F("{\"error\":\"invalid_time\"}");
  } else {
    const String unixText = server.arg("unix_ms");
    const String offsetText = server.arg("utc_offset_minutes");
    char *unixEnd = nullptr;
    char *offsetEnd = nullptr;
    const unsigned long long unixMs = strtoull(unixText.c_str(), &unixEnd, 10);
    const long offset = strtol(offsetText.c_str(), &offsetEnd, 10);
    if (unixEnd == unixText.c_str() || *unixEnd != '\0' ||
        offsetEnd == offsetText.c_str() || *offsetEnd != '\0' ||
        unixMs < 946684800000ULL || offset < -840L || offset > 840L) {
      response = F("{\"error\":\"invalid_time\"}");
    } else if (action.synchronizeTime == nullptr) {
      status = 503; response = F("{\"error\":\"unavailable\"}");
    } else {
      status = action.synchronizeTime(unixMs, static_cast<int16_t>(offset),
                                      response);
    }
  }
  addNoCacheHeaders();
